...
float sceneSDF(vec3 samplePoint) {    
    ...
    samplePoint = infiniteRepetition(samplePoint * rotateX(u_time / 3.0), 13.0 );
    
    float sphere = sphereSDF(vec3(samplePoint.x, samplePoint.y + 3. * -abs(sin(u_time/ 1.5)), samplePoint.z), 2.);
    sphere = unionSDF(sphere, sphereSDF(vec3(samplePoint.x + distance, samplePoint.y + 3. * -abs(sin(u_time/2.)), z), .3));
    sphere = unionSDF(sphere, sphereSDF(vec3(samplePoint.x, samplePoint.y + 3. * -abs(sin(u_time/1.3)), samplePoint.z - distance), .3));
    finalSDF = smoothSubtractionSDF(sphere, finalSDF, 0.8);
    return finalSDF;
}
...
void main()
{
   ...
   // vec3 eye = vec3(5.0, 15.0, 5.0);
    ...
    if (dist > MAX_DIST - EPSILON) {
        // Didn't hit anything
        gl_FragColor = vec4(0.945,0.418,0.736,1.000);
		return;
    }
