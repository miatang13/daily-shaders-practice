...
float sceneSDF(vec3 samplePoint) {    
    ...
    float sphere = sphereSDF(vec3(samplePoint.x, samplePoint.y + 4. * -abs(sin(u_time/ 1.5)), samplePoint.z), 2.);

    finalSDF = smoothSubtractionSDF(sphere, finalSDF, 2.);
    
    float deco = sphereSDF(vec3(samplePoint.x, samplePoint.y + 4. * -abs(sin(u_time/ 1.5)), samplePoint.z), 2.);
