...
float sceneSDF(vec3 samplePoint) {    
    ...
    link = smoothUnionSDF(linkSDF(samplePoint * rotateY(300.0 *sin(u_time * 0.03)), 0.8, 0.9, 0.1) , link, 0.3);
    link = smoothUnionSDF(linkSDF(samplePoint * rotateY(300.0 *cos(u_time * 0.03)), 1.092, 0.2, 0.1) , link, 0.2);
    link = smoothUnionSDF(linkSDF(vec3(samplePoint.x, samplePoint.y + 2.3, samplePoint.z), 0.5, 0.1, 0.2) , link, 0.2);
    
    float thickness = 0.05;
    float offset =sin(1.7*u_time) * 2.0 + 1.5;
    float adjust = 1.5 * abs(sin(u_time)) + 0.3;
